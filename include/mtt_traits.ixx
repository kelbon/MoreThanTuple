...
struct extract_type_list<Template<Types...>> {
  using type = mtt::type_list<Types...>;
};
...
struct merge_type_lists<T<A...>, U<B...>> {
  using type = mtt::type_list<A..., B...>;
};

template <template <typename...> typename T, template <typename...> typename U, typename... A, typename... B,
          typename... Types>
struct merge_type_lists<T<A...>, U<B...>, Types...> {
  using type = typename merge_type_lists<mtt::type_list<A..., B...>, Types...>::type;
};
...
template <template <typename...> typename Template, typename... Types>
struct insert_type_list<Template, mtt::type_list<Types...>> {
...
struct merge_value_lists<mtt::value_list<T, A...>, mtt::value_list<T, B...>, Types...> {
  using type = typename merge_value_lists<mtt::value_list<T, A..., B...>, Types...>::type;
};
template <typename T, T... A, T... B>  // end of recursion
struct merge_value_lists<mtt::value_list<T, A...>, mtt::value_list<T, B...>> {
  using type = mtt::value_list<T, A..., B...>;
};
...
struct value_sequence<T, count, start_value, count, step, Values...> {
  using type = mtt::value_list<T, Values...>;
};
...
struct reverse_type_list_helper<mtt::value_list<int, Indexes...>, Types...> {
 private:
  using help_type = mtt::type_list<Types...>;

 public:
  using type = mtt::type_list<typename help_type::template get<Indexes>...>;
};
...
template <typename... Types>
struct reverse_type_list<mtt::type_list<Types...>> {
...
template <typename... Types>
struct tuple_size<::mtt::type_list<Types...>> : std::integral_constant<size_t, sizeof...(Types)> {};

template <size_t Index, typename... Types>
struct tuple_element<Index, ::mtt::type_list<Types...>> {
  using type = ::mtt::type_of_element_t<Index, Types...>;
};

template <size_t Index, typename... Types>
constexpr auto get(::mtt::type_list<Types...>) noexcept {
  return std::type_identity<::mtt::type_of_element_t<Index, Types...>>{};
}
template <typename T, T... Values>
struct tuple_size<::mtt::value_list<T, Values...>> : std::integral_constant<size_t, sizeof...(Values)> {};

template <size_t Index, typename T, T... Values>
struct tuple_element<Index, ::mtt::value_list<T, Values...>> {
  using type = T;
};

template <size_t Index, typename T, T... Values>
constexpr T get(::mtt::value_list<T, Values...>) noexcept {
  return ::mtt::value_of_element<Index, T, Values...>::value;
}
